...
def _migrar_coluna(conn, tabela: str, coluna: str, definicao: str):
    ...
    if coluna not in colunas:
        ...
        conn.commit()
        logger.info(f"[LTM] Migração: coluna '{coluna}' adicionada em '{tabela}'.")
...
def inicializar():
    ...
    if "conteudo" not in colunas_ep and "resumo" in colunas_ep:
        ...
        conn.commit()
        logger.info("[LTM] Migração: 'resumo' copiado para 'conteudo' em 'episodica'.")
    elif "conteudo" not in colunas_ep:
        conn.execute("ALTER TABLE episodica ADD COLUMN conteudo TEXT NOT NULL DEFAULT ''")
        conn.commit()
        logger.info("[LTM] Migração: coluna 'conteudo' adicionada em 'episodica'.")
...
def limpar_expirados():
    ...
    conn.close()
    if removidos:
        logger.info(f"[LTM] {removidos} episódio(s) expirado(s) removido(s).")


# Inicializa o banco ao importar — limpeza agendada separadamente pelo brain
inicializar()   